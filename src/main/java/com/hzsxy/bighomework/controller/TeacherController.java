...
public class TeacherController implements HandlerInterceptor {
    ...
    public String editClass(Model model, HttpSession httpSession,
        ...
        try {
            Class_Info class_info=class_infoRepository.findOne(class_id);
            class_info.setClass_name(class_name);
            class_info.setTeacher_id_fk(teacherRepository.findOne(teacher_id));
            class_infoRepository.save(class_info);
            return "successful";
        }catch (Exception e){
            e.printStackTrace();
            return "failed";
    ...
    @RequestMapping("/readRequestion")
    public String readRequestion(Model model, HttpServletRequest httpRequest){
        //根据question_id 查询出当前作业的状态
        String question_id = httpRequest.getParameter("question_id");
        List<Student_Submit> submit = student_submitRepository.getByQuestionId(Integer.parseInt(question_id));
        model.addAttribute("submit",submit);
        model.addAttribute("question_id",question_id);
        return "teacher/question-read";
    }
